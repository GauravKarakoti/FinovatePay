...
const { upload, validateFileType } = require('../middleware/upload');
const { authenticateToken, requireRole } = require('../middleware/auth');

// Dispute routes
router.post('/:invoiceId/raise', authenticateToken, requireRole(['buyer', 'seller', 'admin']), disputeController.raiseDispute);
router.post('/:invoiceId/evidence', authenticateToken, requireRole(['buyer', 'seller', 'admin']), upload.single('file'), validateFileType, disputeController.uploadEvidence);
