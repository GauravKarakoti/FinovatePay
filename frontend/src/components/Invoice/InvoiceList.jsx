...
const InvoiceList = ({ invoices, onSelectInvoice, onConfirmRelease, onPayInvoice, onRaiseDispute, onConfirmShipment, userRole }) => {
  ...
  const getStatusBadge = (status) => {
      const statusConfig = {
          ...
          shipped: { label: 'Shipped', color: 'bg-purple-100 text-purple-800' }, // New status
            ...
            {invoices.map((invoice) => (
              <tr key={invoice.invoice_id} className="hover:bg-gray-50">
                <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">
                  {/* FIX: Display a shortened version of the UUID for better readability */}
                  {formatAddress(invoice.invoice_id)}
                </td>
                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                  {/* FIX: Conditionally display the correct counterparty address */}
                  {formatAddress(userRole === 'seller' ? invoice.buyer_address : invoice.seller_address)}
                </td>
                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                ...
                <td className="px-6 py-4 whitespace-nowrap">
                  {getStatusBadge(invoice.escrow_status || invoice.status)}
                </td>
